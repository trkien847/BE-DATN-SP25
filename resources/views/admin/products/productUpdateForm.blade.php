...
<head>
    ...
    <div> 
            <audio id="backgroundMusic" autoplay>
                <source src="{{ asset('audio/Champions 2024.mp3') }}" type="audio/mpeg">
            </audio>
                                            ...
                                            <div class="variant-item bg-white border rounded-lg p-4 hover:shadow-lg transition-all duration-300">
                                                <label class="flex items-center cursor-pointer w-full">
                                                    <input type="checkbox" 
                                                        ...
                                                        {{ collect($selectedVariants)->pluck('shape_id')->contains($value->id) ? 'checked' : '' }}>
    ...
    window.removeVariant = function(combinationId) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            Object.values(existingVariants).forEach(variant => {
                const shapeId = variant.shape_id.toString();
                const shapeName = variant.shape_name;
                const weightId = variant.weight_id.toString();
                ...
                const shapeWeightSet = shapeWeights.get(shapeId);
                shapeWeightSet.add(weightId);
                selectedWeights.add(weightId);
                allSelectedWeights.add(weightId);

                createOrUpdateVariantCard(
                    `${shapeId}-${weightId}`,
                    `${shapeName} ${variant.weight_name}`,
                    variant
                );
            });
            ...
            function createOrUpdateVariantCard(combinationId, combinationName, existingData = null) {
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="number" 
                                        ...
                                        value="${existingData ? existingData.sale_price || '' : ''}"
                                        class="form-control w-full mt-1" 
                                        min="0">
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="datetime-local" 
                                        ...
                                        value="${existingData ? existingData.sale_price_start_at || '' : ''}"
                                        class="form-control w-full mt-1">
                                </div>
                                <div>
                                    ...
                                    <input type="datetime-local" 
                                        ...
                                        value="${existingData ? existingData.sale_end_at || '' : ''}"
                                        class="form-control w-full mt-1">
                                </div>
                            </div>
                        </div>
            ...
            selectedShapes.forEach((_, shapeId) => {
                const checkbox = document.querySelector(`input[value="${shapeId}"].variant-checkbox-input`);
                if (checkbox) {
